...
const RevisarPedido = () => {
        ...
        if (carrito.length > 0 && usuario?.sucursal_codigo && hayNuevo && reglasConvenios) {
            const fetchData = async () => {
                ...
                carrito.forEach((item) => {
                    ...
                    const stockDepoItem = stock.find(s => s.ean === item.ean)?.stock ?? 0;
                    if (stockDepoItem > 0) {
                    ...
                    if (match.aplica) {
                        // prioridad estricta (puede incluir "deposito" y "kellerhoff")
                        ...
                        // ninguna opción viable y el primero no es externo → Falta
                        seleccionInicial[item.ean] = { proveedor: "Falta", motivo: "Falta" };
                        return;
                    }
    ...
    useEffect(() => {
        ...
        carrito.forEach((item) => {
            ...
            // 👇 si estaba como Falta y ahora hay una opción válida, auto-switch a mejor
    ...
    function proveedorViable(slug, ean, ctx) {
        ...
        if (slug === "kellerhoff") {
            console.debug("🔎 viable? kellerhoff", { ean, ok: true });
            return true;
        }
        const fuente =
    ...
    function pickPorPrioridad(item, prioridad, ctx) {
        ...
        if (prioridad[0] === "kellerhoff") {
            console.debug("ninguno viable; devolvemos kellerhoff igual para que prueben en web");
            console.groupEnd();
            return "kellerhoff";
        }
        console.debug("❌ ninguno viable → Falta");
        console.groupEnd();
        return null;
    }
                            ...
                            {carrito.map((item) => {
                                ...
                                return (
                                    <tr key={item.ean} className={estaNoPedir ? "fila_omitida" : ""}>
                                        ...
                                        <td className={"celda_kellerof" + (seleccion[item.ean]?.proveedor === "kellerhoff" ? " celda_activa" : "")}>
                                            <PreciosKellerof
                                                ean={item.ean}
                                                seleccionado={seleccion[item.ean]?.proveedor === "kellerhoff"}
                                                onSelect={handleElegirProveedor}
                                            />
                                        </td>
                                        ...
                                        <td>
                                            <select
                                                ...
                                                disabled={estaNoPedir || motivoBloqueado || motivoActual === "Falta"}
                                            >
                                                {opcionesMotivo.map((op) => {
                                                    const isBlocked =
                                                        (op.value === "Stock Depo" && (proveedorActual !== "deposito" || stockDepo <= 0)) ||
                                                        (op.value === "Falta" && tieneAlgunoConPrecio);
                                                    return (
